...
# class split_noniid_with_rho():
#     def __init__(self, alpha, run_path, is_plot, seed, rho):
#         self._alpha = alpha
#         self._rho = rho  # Add rho parameter
#         self._run_path = run_path
#         self._is_plot = is_plot
#         self._seed = seed
#
...
#         idx = [[int(i) for i in item] for item in idx]
#
#         # Determine the number of main-distribution classes according to rho
#         n_main_classes = int(n_classes * (1 - self._rho))
#         n_ood_classes = n_classes - n_main_classes
#
#         # Calculate the number of samples per class for each device (Original)
#         n_samples_per_class = [len(item) for item in idx]
#
#         s = np.stack([rng.multinomial(int(n_samples_per_class[i]),
#                                       rng.dirichlet([1*self._alpha for _ in range(n_device)])) for i in range(n_classes)]).tolist()
#         # Allocate the samples to each device
#         new_ordered_idxs = []
#         ii = list(range(n_classes))
#         jj = list(range(n_device))
#
...
class split_noniid_with_rho():
    ...
    def __call__(self, dataset, n_device):
        ...
        n_classes = len(dataset.dataset.classes)
        len_dataset = len(dataset.dataset.targets)
        n_data_per_device = int(len_dataset * (1 - self._rho) / n_device)
        ...
        # Randomly select main-distribution classes and OOD classes
        main_classes = rng.choice(n_classes, n_main_classes, replace=False)
        ood_classes = np.array([i for i in range(n_classes) if i not in main_classes])

        main_idx = [[idx[i]] for i in main_classes]
        ood_idx = [idx[i] for i in ood_classes]

        # Flatten the lists
        main_idx = [item for sublist in main_idx for item in sublist]
        ood_idx = [item for sublist in ood_idx for item in sublist]

        # Calculate the number of samples per class for each device
        n_samples_per_class = [len(item) for item in main_idx]
        s = np.stack([rng.multinomial(int(n_samples_per_class[i]),
                                      rng.dirichlet([1 * self._alpha for _ in range(n_device)])) for i in
                      range(n_main_classes)]).tolist()

        # Allocate the samples to each device
        new_ordered_idxs = []
        ii = list(range(n_main_classes))
        jj = list(range(n_device))

        for j in jj:
            for i in ii:
                new_ordered_idxs.append(main_idx[i][0:s[i][j]])
                main_idx[i] = main_idx[i][s[i][j]:]
        ...
        return dev_idxs  # Return the indices of the OOD samples for testing
