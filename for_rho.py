...
if __name__ == "__main__":
    ...
    # setting rho
    parser.add_argument("--rho", type=float, default=0.4, help="Ratio of out-of-distribution test samples to main test samples")
    parser.add_argument("--session_tag", type=str, default="default_session", help="Sets name of subfolder for experiments")
    parser.add_argument("--algorithm", type=str, default="CoCoFL",
    ...
    parser.add_argument("--data_distribution", type=str, choices=['IID', 'NONIID', 'RCNONIID','NONIID_RHO'], default="NONIID_RHO")
    ...
    # parser.add_argument("--torch_device", type=str, default="cpu", help="PyTorch device (cpu)")
    ...
    # parser.add_argument("--dry_run", default=True, help="Dry_run loads the NN, datasets, but does not apply training and does not create any files")
    ...
    if 'CIFAR' in args.dataset:
        ...
        if args.dataset.endswith('100'):
            flserver.set_dataset(datasets.CIFAR100, "/root/CoCoFL-master/dataset/", **kwargs)
            cnn_args = {'num_classes': 100}
        elif args.dataset.endswith('10'):
            flserver.set_dataset(datasets.CIFAR10, "/root/CoCoFL-master/dataset/", **kwargs)
        ...
        kwargs = {'download': True, 'transform': tf}
        flserver.set_dataset(CINIC10, "/root/CoCoFL-master/dataset/", **kwargs)
        cnn_args = {'num_classes': 10}
        ...
        kwargs = {'download': True, 'transform': tf}
        flserver.set_dataset(XCHEST, "/root/CoCoFL-master/dataset/", **kwargs)
    ...
    from utils.split import split_iid, split_noniid, split_rcnoniid, split_SHAKESPEARE_rcnoniid,split_noniid_with_rho
    ...
    elif args.data_distribution == 'NONIID_RHO':
        flserver.split_function = split_noniid_with_rho(args.noniid_dirichlet, run_path, False if args.dry_run else args.plot, args.seed,args.rho)
